...
export default async function seedDemoData({ container }: ExecArgs) {
  ...
  const firstNames = ["John", "Jane", "Alice", "Bob", "Charlie", "David", "Eve"];
  ...
  for (let i = 0; i < 50; i++) {
    ...
    logger.info(`Created customer ${first_name} ${last_name} with email ${email}`);
  ...
  logger.info("Start seeding order data");
  ...
  for (const order of orders) {
    ...
    try {
      ...
      const { data: orderData } = await query.graph({
        entity: "order",
        fields: ["*","summary.*","shipping_methods.*","shipping_address.*","billing_address.*", "items.*"],
        filters: {
          id: result.id,
        },
      });
      ...
      const { result: paymentCollection } = await createOrderPaymentCollectionWorkflow(container).run({
        input: {
          ...
          amount: orderData[0].summary?.pending_difference as number,
        }
      });

      logger.info(`Created payment collection of order with ID: ${result.id}`);
      
      const { data: paymentCollectionData } = await query.graph({
      ...
      const { result: paymentSessions } = await createPaymentSessionsWorkflow(container).run({
        input: {
          payment_collection_id: paymentCollectionData[0].id,
          provider_id: "pp_system_default", // Assuming you're using Stripe
        }
      });

      logger.info(`Created payment session of order with ID: ${result.id}`);

      const { data: paymentSessionData } = await query.graph({
      ...
      const { result: processedPayment } = await processPaymentWorkflow(container).run({
        input: {
          action: "captured",
          data: {
            session_id: paymentSessionData[0].id,
            amount: paymentSessionData[0].amount as number,
          }
        }
      });

      const { result: markedAsPaid } = await markPaymentCollectionAsPaid(container).run({
        input: {
          order_id: orderData[0].id,
          payment_collection_id: paymentCollectionData[0].id,
        }
      ...
      logger.info(`Marked order ${orderData[0].id} as paid`);
      
      logger.info(`Processed payment of order with ID: ${result.id}`);
  ...
  logger.info("Finished seeding order data.");
}
